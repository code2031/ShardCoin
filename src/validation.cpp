    ...
    for (const CTxInput& txin : tx.GetInputs())
    {
        ...
        if (ptxConflicting) {
            if (!setConflicts.count(ptxConflicting->GetHash()))
            {
                ...
                // ShardCoin: Only support BIP125 RBF when -mempoolreplacement arg is set
...
{
    // ShardCoin: Smooth reward decay
    // Reward decreases by 10% every 100,000 blocks (~173 days at 2.5 min blocks)
    // Block 0:       5 SHRD
    // Block 100000:  4.5 SHRD
    // Block 200000:  4.05 SHRD
    // Block 500000:  2.952 SHRD
    // Block 1000000: 1.743 SHRD
    // Block 2000000: 0.608 SHRD
    // Total supply: ~8.4M SHRD

    const int decayInterval = 100000; // blocks between each reduction
    int reductions = nHeight / decayInterval;

    CAmount nSubsidy = 5 * COIN;

    // Apply 10% reduction for each interval (multiply by 0.9 each time)
    for (int i = 0; i < reductions; i++) {
        nSubsidy = nSubsidy * 9 / 10;
        if (nSubsidy < 1) return 0; // minimum 1 satoshi, then zero
    }
