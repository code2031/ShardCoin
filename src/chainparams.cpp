...
/**
 * Mine the genesis block by finding a nonce that produces a hash below the target.
 * Called during initialization — logs results via LogPrintf (not printf).
 */
static void MineGenesisBlock(CBlock& genesis, const uint256& powLimit)
{
...
{
    const char* pszTimestamp = "ShardCoin: A new era of decentralized currency - March 2026";
...
public:
    CMainParams() {
        ...
        consensus.BIP16Height = 0;   // ShardCoin: Activate all BIPs from genesis
        ...
        // Deployment of MWEB (LIP-0002, LIP-0003, LIP-0004) - active from genesis
        ...
        genesis = CreateGenesisBlock(1711036800, 0, 0x1e0ffff0, 1, 50 * COIN);
        // Mine the genesis block on first run
        MineGenesisBlock(genesis, consensus.powLimit);
        consensus.hashGenesisBlock = genesis.GetHash();

        vSeeds.clear(); // ShardCoin: No DNS seeds yet - add your own seed nodes here
        ...
        vFixedSeeds.clear(); // ShardCoin: No fixed seeds yet
...
public:
    CTestNetParams() {
        ...
        genesis = CreateGenesisBlock(1711036801, 0, 0x1e0ffff0, 1, 50 * COIN);
        // Mine the testnet genesis block on first run
        MineGenesisBlock(genesis, consensus.powLimit);
        consensus.hashGenesisBlock = genesis.GetHash();
...
public:
    explicit CRegTestParams(const ArgsManager& args) {
        ...
        genesis = CreateGenesisBlock(1711036802, 0, 0x207fffff, 1, 50 * COIN);
        // Mine the regtest genesis block on first run
        MineGenesisBlock(genesis, consensus.powLimit);
        consensus.hashGenesisBlock = genesis.GetHash();
        ...
        checkpointData = {
            {
                // {0, uint256S("...")}, // TODO: Fill in after mining genesis
            }
        };
