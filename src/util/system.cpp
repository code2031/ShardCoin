...
const char * const BITCOIN_CONF_FILENAME = "shardcoin.conf";
...
#else
    const char* pszModule = "shardcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ShardCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/ShardCoin";
#else
    // Unix-like
    return pathRet / ".shardcoin";
#endif
#endif
}
