...
The ShardCoin daemon can be configured to act as a trusted "border
router", implementing the shardcoin wire protocol and relay, making
...
The ZeroMQ feature in ShardCoin requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building shardcoind:
...
For instance:
    ...
    $ shardcoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/shardcoind.tx.raw \
...
These options can also be provided in shardcoin.conf.
...
From the perspective of shardcoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into shardcoind directly. Furthermore, no information is
...
using. ShardCoind appends an up-counting sequence number to each
